...
from fsmfuncs import advection_hat 

def linearize_advection_hat (dx, dy, dxp, dyp, U0, uS, uN, uW, uE, vS, vN, vW, vE, test=False):
    n, m = len(dy), len(dx)
    Nu0, Nv0 = advection_hat(dx, dy, dxp, dyp, U0, uS, uN, uW, uE, vS, vN, vW, vE)
    h = 1e-8j
    ...
    for idxj in range(sw):
      for idxi in range(sw):
        ...
        vidx = -np.ones((n-1, m), dtype=int)
        for svuk in svu:
            jj, ii = idx[0]+svuk[0], idx[1]+svuk[1]
            mask = (0<=jj)*(jj<(n-1))*(0<=ii)*(ii<m)
            vidx[jj[mask], ii[mask]] = tmpidx[mask]

        U = np.concatenate([u.ravel(), v.ravel()])
        α=la.norm(U, np.inf)
        Nu1, Nv1 = advection_hat(dx, dy, dxp, dyp, U0+h/α*U, uS, uN, uW, uE, vS, vN, vW, vE)
